...
<template>
  <div class="metrics-strip">
    ...
    <div><span>5日收益</span><strong>{{ percent(metrics?.returns['5d']) }}</strong></div>
    <div><span>20日收益</span><strong>{{ percent(metrics?.returns['20d']) }}</strong></div>
    <div><span>年化波动</span><strong>{{ percent(metrics?.annualized_volatility) }}</strong></div>
    <div><span>最大回撤</span><strong>{{ percent(metrics?.max_drawdown) }}</strong></div>
  </div>
</template>
